...
%
% FORMAT:
% [bnpl,varb,pchoice,v1npl,vnpl,iter,err,times] = npldygam(aobs,zobs,aobs_1,zval,ptranz,pchoice,bdisc,miniter,maxiter,verbose)
%
% INPUTS:
...
%     varb    - (kparam x kparam) varb matrix from clogit estimation.
%     pchoice - ((numz*2^nplayer) x nplayer) matrix of players'
...
function [ bnpl, varb, pchoice, v1npl, vnpl, llike, iter, err, times ] = npldygam(aobs, zobs, aobs_1, zval, ptranz, pchoice, bdisc, miniter, maxiter, verbose)
  % ---------------
  % Some constants
  % ---------------
  eulerc = 0.5772;
